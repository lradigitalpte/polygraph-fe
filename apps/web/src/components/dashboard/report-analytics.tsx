...
export function ReportAnalytics({ appointments, shares, examiners = [], stats, loading, canFilterExaminer }: Props) {
  ...
  const examinerNames = React.useMemo(() => new Map(examiners.map((item) => [item.id, item.name])), [examiners]);
  const examinerName = React.useCallback((id: number) => examinerNames.get(id) || `Examiner #${id}`, [examinerNames]);
  ...
  const exportCsv = () => {
    ...
    const rows = filtered.map((item) => [item.id, item.scheduled_at, examinerName(item.examiner_id),
  ...
  return <motion.section initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .4 }} className="space-y-6">
      ...
      <CardContent className="flex flex-col gap-4 p-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          {canFilterExaminer && <select aria-label="Filter by examiner" value={examiner} onChange={(event) => setExaminer(event.target.value)} className="h-9 rounded-xl border border-border bg-background px-3 text-sm font-semibold outline-none focus:ring-2 focus:ring-primary/30">
            <option value="all">All examiners</option>{examinerIds.map((id) => <option key={id} value={id}>{examinerName(id)}</option>)}
          </select>}
    ...
    <div className="grid gap-6 xl:grid-cols-[1.35fr_.9fr]">
      ...
      <Card className="rounded-3xl border-border/50"><CardHeader className="flex-row items-center justify-between"><div><CardTitle className="text-base">Examiner performance</CardTitle><p className="text-xs text-muted-foreground">Booked and completed</p></div><UserRoundSearch className="h-5 w-5 text-primary" /></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2">{examinerVolume.length ? examinerVolume.map((item) => <div key={item.id} className="rounded-2xl border border-border/50 bg-muted/25 p-4"><p className="text-xs font-bold text-muted-foreground">{examinerName(item.id)}</p><div className="mt-3 flex items-end justify-between"><b className="text-2xl">{item.completed}/{item.booked}</b><span className="text-[10px] font-bold uppercase text-muted-foreground">{pct(item.completed, item.booked)}% complete</span></div></div>) : <p className="col-span-2 py-8 text-center text-sm text-muted-foreground">No examiner activity in this period.</p>}</CardContent></Card>
